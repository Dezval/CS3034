...
node* create(monsterAttack attackData,node* next)
{
    ...
    new_node->attackData = attackData;
//    new_node->data = data;
    new_node->next = next;
...
{
    node* new_node = create(attackData,head);
    head = new_node;
    return head;
}
...
{
	if(head == NULL) head = create(attackData, head);
	else {
		/* go to the last node */
		node *cursor = head;
		...
		/* create a new node */
		node* new_node =  create(attackData,NULL);
...
node* insert_after(node *head, monsterAttack attackData, node* prev)
{
    ...
    if(cursor != NULL)
    {
        node* new_node = create(attackData,cursor->next);
        cursor->next = new_node;
        return head;
    }
    else
...
void display(node* n) {
    if (n != NULL){
        printf("ID: %d\n", n->attackData.attackID);
        printf(" | Location: %s", n->attackData.attackLocation);
        printf(" | Monster: %s", n->attackData.monsterName);
        printf(" | Victims: %d\n", n->attackData.victims);
    }
}
...
node* search(node* head,int attackID)
{
    ...
    while(cursor!=NULL)
    {
        if(cursor->attackData.attackID == attackID)
            return cursor;
        cursor = cursor->next;
    }
    return NULL;
...
node* insertion_sort(node* head)
{
    ...
    while(x != NULL)
    {
        ...
        if (head != NULL)
        {
            if(e->attackData.attackID > head->attackData.attackID)
            {
                y = head;
                while ((y->next != NULL) && (e->attackData.attackID> y->next->attackData.attackID))
                {
                    y = y->next;
                }
                e->next = y->next;
