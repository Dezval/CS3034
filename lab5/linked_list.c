...
*/
void traverse(node* head, void(*printFunc)()) {
//    printf("Traverse 1");
    node* cursor = head;
    while(cursor != NULL)
    {
        display(cursor,printFunc);
        cursor = cursor->next;
    }
    printf("\n");
}
...
/*
    display a node
*/
void display(node* n, void(*printFunc)()) {
//    printf("DISPLAY 1\n");
    if (n != NULL) {
        printFunc(n->data);
    }
...
*/
node* search(node* head,int ID, int(*getID)()) {
 
    node *cursor = head;
    while(cursor!=NULL)
    {
        if( getID(cursor->data) == ID )
            return cursor;
        cursor = cursor->next;
    }
    return NULL;
...
*/
node* insertion_sort(node* head, int(*getID)()) {
    node *x, *y, *e;
    ...
    while(x != NULL)
    {
        ...
        if (head != NULL)
        {
            if( getID(e->data) > getID(head->data) )
            {
                y = head;
                while ((y->next != NULL) && ( getID(e->data) > getID(y->next->data) ))
                {
                    y = y->next;
                }
                e->next = y->next;
